...
import io
from reportlab.lib.utils import ImageReader  # Importamos ImageReader
...
class ImageProcessorApp:
    ...
    def create_widgets(self):
        ...
        self.cell_slider = tk.Scale(self.root, from_=4, to_=8, orient="horizontal", resolution=1,
                                    label="Cells per Page")
        self.cell_slider.set(6)  # Default value for 6x6 grid
    ...
    def processImages(self):
        ...
        for item in os.listdir(self.input_folder):
            if item.lower().endswith(".png"):
                ...
                im.save(img_byte_arr, format='JPEG', quality=90)
                img_byte_arr.seek(0)
                images.append(img_byte_arr)

        return images

    def createPDF(self, images):
        ...
        images_per_page = self.columns * self.rows  # Número máximo de imágenes por página

        for idx, img_byte_arr in enumerate(images):
            if idx > 0 and idx % images_per_page == 0:
                c.showPage()  # Crear una nueva página si se alcanza el límite de imágenes por página
                x_offset = margin
                y_offset = page_height - margin - cell_height

            # Convertimos el objeto BytesIO en una imagen compatible con ReportLab
            img_reader = ImageReader(img_byte_arr)
            c.drawImage(img_reader, x_offset, y_offset, width=cell_width, height=cell_height)
    ...
    def generate_pdf(self):
        images = self.processImages()
        if images:
            self.createPDF(images)
